...
public class RegistroServiceImpl implements RegistroService {
	...
	public Model activar(Model model, String username, String clave) {
		Usuario usuario =
				usuarioService.getUsuarioPorUsernameYClave(username,
						clave);
		if (usuario != null) {
	...
	public void activar(Usuario usuario) {
		...
		usuario.setClave(codigo.encode(usuario.getClave()));
	...
	private String demeClave() {
		...
		for (int i = 0; i < 40; i++) {
			int index = random.nextInt(tira.length());
			clave.append(tira.charAt(index));
		}
	...
	private void enviaCorreoActivar(Usuario usuario, String clave) throws MessagingException {
		// Obtiene el mensaje de la fuente de mensajes
		String mensajeTemplate = messageSource.getMessage(
				"registro.correo.activar",
				null, Locale.getDefault());

		// Asegúrate de que el mensaje tenga los especificadores de formato correctos
		String mensaje = String.format(
				mensajeTemplate,
				usuario.getNombre(),
				servidor,  // Asegúrate de que 'servidor' contenga la URL base del servidor
				usuario.getUsername(),
				clave
		);

		// Obtiene el asunto del mensaje de la fuente de mensajes
		String asunto = messageSource.getMessage(
				"registro.mensaje.activacion",
				null, Locale.getDefault()
		);

		// Envía el correo electrónico usando el servicio de correo
